...
template <class type>
class matrixWithPartialGauss:public matrix<type>{
public:///Constructors and destructor
    matrixWithPartialGauss(){
        matrix<type>::n = 0;
        matrix<type>::m = 0;
        matrix<type>::item = NULL;
    }
    matrixWithPartialGauss(int nn, int mm){
        matrix<type>::n = nn;
        matrix<type>::m = mm;
        matrix<type>::create();
    }
    matrixWithPartialGauss(int nn){
        matrix<type>::n = nn;
        matrix<type>::m = nn;
        matrix<type>::create();
    }
    matrixWithPartialGauss(const matrixWithPartialGauss<type> &rhs){
        matrix<type>::n = rhs.n;
        matrix<type>::m = rhs.m;
        matrix<type>::create();
    ...
    ~matrixWithPartialGauss(){}
public:///find determinant and do gauss
    void gaussPartialRow(bool showMatrixChange);
    void gaussPartialColumn(bool showMatrixChange);
};

...
////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////
template <class type> void          matrixWithPartialGauss<type>::  gaussPartialRow(bool showMatrixChange){
    int IDmaxFromColumn;
    type item_a_a, item_b_a;

    if (matrix<type>::n >= matrix<type>::m){
        for (int a = 1; a <= matrix<type>::m - 1; a++){
            IDmaxFromColumn = matrix<type>::indexMaxFromColumn(a,a) + 1;
            if (a >= matrix<type>::m - 2) IDmaxFromColumn = a;
            if (IDmaxFromColumn != a) matrix<type>::swapRows(a,IDmaxFromColumn);
            if (showMatrixChange) matrix<type>::print();

            for (int b = a + 1; b <= matrix<type>::n; b++){

                if (matrix<type>::item[a - 1][a - 1] == 0)
                    matrix<type>::swapRows(a,b);
                if (matrix<type>::item[b - 1][a - 1] == 0)
                    continue;

                item_a_a = matrix<type>::item[a - 1][a - 1];
                item_b_a = matrix<type>::item[b - 1][a - 1];

                matrix<type>::multRow(a,item_b_a);
                if (showMatrixChange) matrix<type>::print();
                matrix<type>::multRow(b,item_a_a);
                if (showMatrixChange) matrix<type>::print();
                matrix<type>::subRowToRow(b,a);
                if (showMatrixChange) matrix<type>::print();
                matrix<type>::divRow(a,matrix<type>::gcf_allRow(a));
                if (showMatrixChange) matrix<type>::print();
                matrix<type>::divRow(b,matrix<type>::gcf_allRow(b));
                if (showMatrixChange) matrix<type>::print();
                if (showMatrixChange) cout << "-------------------------------------------------------" << endl;
            }
        }
        ...
        for (int a = 1; a <= matrix<type>::n - 1; a++){

            IDmaxFromColumn = matrix<type>::indexMaxFromColumn(a,a) + 1;
            if (a >= matrix<type>::n - 2) IDmaxFromColumn = a;
            if (IDmaxFromColumn != a) matrix<type>::swapRows(a,IDmaxFromColumn);
            if (showMatrixChange) matrix<type>::print();
            ...
            for(int b = a + 1; b <= matrix<type>::n; b++){
                ...
                if (matrix<type>::item[b - 1][a - 1] == 0)
                    ...
                    matrix<type>::multRow(a, item_b_a);
                    if (showMatrixChange == true) matrix<type>::print();
                    matrix<type>::multRow(b, item_a_a);
                    if (showMatrixChange == true) matrix<type>::print();
...
template <class type> void          matrixWithPartialGauss<type>::  gaussPartialColumn(bool showMatrixChange){
    matrixWithPartialGauss <type> a(this->n,this->m);
    a = *this;
    a.transpose();
    a.gaussPartialRow(showMatrixChange);
    a.transpose();
    *this = a;
    a.destroyMatrix();
}
...
int main()
{
    matrixWithPartialGauss <int> a(7);
    a.randFill(0,2);
    a.print();
    a.gaussPartialColumn(true);
    a.print();
    return 0;
}
