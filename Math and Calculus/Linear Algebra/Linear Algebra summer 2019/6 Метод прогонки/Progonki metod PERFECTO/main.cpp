...
class matrixTRIAG:public matrix{
public:///Constructors and destructor
    matrixTRIAG(){
        matrix::n = 0;
        matrix::m = 0;
    ...
    matrixTRIAG(const matrixTRIAG &rhs){
        matrix::n = rhs.n;
        matrix::m = rhs.m;
        matrix::create();
    ...
    ~matrixTRIAG(){}
public:///create triangonal matrix, progonki method
    void createTRIAGSolve(double min, double max);
    void progonkiMethodSolve(const matrixTRIAG &bcol);
};

void matrixTRIAG::createTRIAGSolve(double min, double max){
    if (n + 1 != m) return;
    for (int a = 0; a < n; a++){
        for (int b = a - 1; b <= a + 1; b++){
            if (b == -1){
                item[a][0] = double(rand()%(int)max) + min;
                continue;
            }
            if (b == m){
                break;
            }
            item[a][b] = double(rand()%(int)max) + min;
        }
        item[a][m - 1] = double(rand()%(int)max) + min;
    }
}
void matrixTRIAG::progonkiMethodSolve(const matrixTRIAG &x){
    if (n + 1 != m) return;
    double a[n];
    double b[n];
    double c[n];
    double f[n];
    for (int k = 0; k < n; k++){
        if (k > 0) a[k] = item[k][k - 1];
        if (k < n - 1) b[k] = item[k][k + 1];
        c[k] = item[k][k];
        f[k] = item[k][m - 1];
    }
	...
	x.item[n-1][0] = f[n-1]/c[n-1];

	for (int i = n - 2; i >= 0; i--)
    {
		x.item[i][0]=(f[i]-b[i]*x.item[i+1][0])/c[i];
    }
...
int main(){
    int num = 3;
    matrixTRIAG A(num,num+1);
    matrixTRIAG ans(num, 1);
    A.createTRIAGSolve(1,4);
    A.progonkiMethodSolve(ans);
    A.print();
    ans.print();
