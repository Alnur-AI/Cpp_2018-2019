...
public: ///Math matrix functions
    ///FIND MAX ELEMENT
    int indexMaxFromColumn(int j);//Возвращает индекс наибольшего числа в столбце
    int indexMaxFromRow(int i);//Возвращает индекс наибольшего числа в строке
...
////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////
template <class type> int  matrix<type>:: indexMaxFromColumn(int j)
{
    if (j <= 0)
    {
...
template <class type> int  matrix<type>:: indexMaxFromRow(int i)
{
    if (i <= 0)
    {
...
int main()
{
    matrix <long long> A(8,10);
    A.randFill(0,3);
    A.print();
    A.gaussSimpleAnyTypeDOWNcolumn(true);
        ...
        for (int a = 1; a <= n - 1; a++)
            for(int b = 1; b <= m; b++)
                if (b > a)
                {
                    ...
                    multColumn(a, item_a_b);
                    if (showMatrixChange == true) print();
                    multColumn(b, item_a_a);
                    if (showMatrixChange == true) print();

                    subColumnToColumn(b,a);
                    if (showMatrixChange == true) print();
                    divColumn(a, gcf_allCol(a));
                    if (showMatrixChange == true) print();
                    divColumn(b, gcf_allCol(b));
                    if (showMatrixChange == true) print();
                    if (showMatrixChange == true) cout << "-------------------------------------------------------" << endl;
                }
    }
